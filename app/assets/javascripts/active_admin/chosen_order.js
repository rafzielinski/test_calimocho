...
;(function () {
  ...
  AbstractChosenOrder = (function () {
    ...
    ERRORS = {
      ...
      ordering_unselected_option:
        "ChosenOrder::{{function}}: ignoring option '{{option}}' which is not selected. Set optional parameter \"force\" to 'true' to get the ordered selection forced first.",
    }
    ...
    AbstractChosenOrder.getFlattenedOptionsAndGroups = function (select) {
      ...
      for (_i = 0, _len = options.length; _i < _len; _i++) {
        ...
        if (opt.nodeName.toUpperCase() === 'OPTGROUP') {
          sub_options = Array.prototype.filter.call(opt.childNodes, function (
            o
          ) {
    ...
    AbstractChosenOrder.setSelectionOrder = function (select, order, force) {
      ...
      if (order instanceof Array) {
        ...
        for (i = _i = 0, _len = order.length; _i < _len; i = ++_i) {
          ...
          if (option == null) {
            console.warn(
              ERRORS.ordering_unselected_option
                .replace('{{function}}', 'setSelectionOrder')
          ...
          _results.push(
            this.insertAt(
              option,
              i,
              chosen_ui.querySelector('ul.chosen-choices')
            )
          )
        }
        return _results
